...
from app_restapi.models import Booking, Room
from app_restapi.serializers import BookingSerializer, RoomSerializer
...
logger.info("Initializing the Rooms Booking API service Main engine...")
...
class Home(generics.ListCreateAPIView):
	...
	def get(self, request, format=None):
		...
		return Response({'success': True,'Message':"Welcome to the Room Booking App!.."}, status=status.HTTP_200_OK)


class AllRoomsList(generics.ListCreateAPIView):
	if settings.USE_TOKEN:
		permission_classes = (IsAuthenticated, )
		authentication_classes = (JSONWebTokenAuthentication, )

	# logger.info("Calling rooms list endpoint(GET/POST) url")
